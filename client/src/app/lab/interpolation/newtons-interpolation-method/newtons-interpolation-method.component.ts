...
@UntilDestroy()
@Component({
  selector: 'nml-newtons-interpolation-method',
  templateUrl: './newtons-interpolation-method.component.html',
})
export class NewtonsInterpolationMethodComponent implements OnInit {
  ...
  submit(): void {
    ...
    if (!this.xValue.value) this.xValue.setValue(DEFAULT_X_VALUE, { emitEvent: false });
    this.store.dispatch(new UpdateLabInput(this.lab, this.inputForm.value));
    this.store.dispatch(
